...
while(1) {
	...
	warn join(",",@values);
	my $format = " %0.03f "x(scalar(@values));
	my $entropy = balance_entropy( @values );
	my @entropies = map { my @a = @$_; entropy( @a ); } @scale;
	#my @ve = zip( \@values, \@entropies );
	my $sum = sum @values;
	my @scaled = map { 1.0*$_ / $sum } @values;

	my @out = ();
	push @out, sprintf("i\"setEntropy\" 0.0001 1 %0.03f", $entropy);
	push @out, sprintf("i\"setValues\" 0.0001 1 $format", @values);
	push @out, sprintf("i\"setRelValues\" 0.0001 1 $format",@scaled);
	push @out, sprintf("i\"setEntropies\" 0.0001 1 $format", @entropies);
	my $str = join($/,@out).$/;
	warn $str;
	print $str;
	sleep( 0.25 );
